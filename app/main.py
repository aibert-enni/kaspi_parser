...
async def kaspi_products_scrapping():
    ...
    for url in urls:
        ...
        try:
            ...
            async with get_product_service() as product_service:
                ...
                if product_db:
                    if datetime.now(timezone.utc) - product_db.updated_at < timedelta(
                        minutes=15
                    ):
                        logger.info(
                            f"Product with code {product_code} already exists and was updated less than 15 minutes ago. Skipping..."
                        )
                ...
                if product_db:
                    product_db = await product_service.update_by_difference(
                        original=product_db, new=product_new
                    )
    ...
    export_data = {"products_info": products_info}
